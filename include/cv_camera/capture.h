...
public:
  ...
  */
   ...
   *        from the previous call, push the unchanged/changed result into the sliding
   *        window, and refresh the reference frame for the next call. Intended to be
   *        called from a dedicated timer (not every capture() cycle).
   */

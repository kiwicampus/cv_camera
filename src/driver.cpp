...
void Driver::parameters_setup()
{
  ...
  param_manager_.addParameter(rotate_, "rotate", 0);
...
void Driver::proceed()
{
  if (video_path_ != "") camera_->capture(flip_vertical_, flip_horizontal_, rotate_);
  ...
  {
    if (!camera_->capture(flip_vertical_, flip_horizontal_, rotate_))
    {
...
void Driver::attempt_reconnection()
{
  ...
  reconnection_tmr_ = this->create_wall_timer(std::chrono::seconds(video_stream_recovery_time_), [this]() {
    ...
    {
      if (camera_->grab() && camera_->capture(flip_vertical_, flip_horizontal_, rotate_))
      {
        reconnection_tmr_->cancel();
        reconnection_tmr_.reset();

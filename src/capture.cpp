...
bool Capture::grabLatest(cv::OutputArray& image){
    if (!cap_.isOpened()){
        return false;
    }
    while (1) {
        ...
        cap_.grab();
        if (std::chrono::duration_cast<std::chrono::duration<float>>(std::chrono::high_resolution_clock::now() - time_start).count() * cap_.get(cv::CAP_PROP_FPS) > 0.5) {
            break;
        }
    }
    cap_.retrieve(image);
